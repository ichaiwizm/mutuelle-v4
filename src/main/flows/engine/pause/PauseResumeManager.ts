...
type PauseResumeConfig = { enabled: boolean; stateId?: string };

export class PauseResumeManager {
  private currentState?: FlowState;
  private pauseRequested = false;

  constructor(private config: PauseResumeConfig, private stateService: FlowStateService) {}

  get isEnabled(): boolean { return this.config.enabled; }
  get state(): Readonly<FlowState> | undefined { return this.currentState ? { ...this.currentState } : undefined; }
  get isPauseRequested(): boolean { return this.pauseRequested; }
  ...
  async initialize(flowKey: string, leadId?: string, logger?: FlowLogger): Promise<number> {
    ...
    if (this.config.stateId) {
      ...
      if (!loadedState) throw new Error(`Flow state not found: ${this.config.stateId}`);
      ...
      await this.stateService.updateState(this.currentState.id, { status: "running", resumedAt: Date.now() });
  ...
  async checkpoint(stepIndex: number, stepId: string): Promise<void> {
    ...
    await this.stateService.updateState(this.currentState.id, { currentStepIndex: newStepIndex, completedSteps: newCompletedSteps });
    this.currentState = { ...this.currentState, completedSteps: newCompletedSteps, currentStepIndex: newStepIndex };
  }

  async markPaused(): Promise<void> {
    if (this.currentState) await this.stateService.markPaused(this.currentState.id);
  }

  async markCompleted(): Promise<void> {
    if (this.currentState) await this.stateService.markCompleted(this.currentState.id);
  }

  async markFailed(): Promise<void> {
    if (this.currentState) await this.stateService.markFailed(this.currentState.id);
  }

  reset(): void { this.pauseRequested = false; }
}
