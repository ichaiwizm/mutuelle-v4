/**
 * Transformateur principal pour Alptis Santé Pro Plus
 *
 * Transforme un Lead AssurProspect en données formatées pour Alptis Santé Pro Plus
 *
 * Différences avec Santé Select:
 * - Éligibilité: TNS uniquement (pas age >= 60)
 * - Adhérent: micro_entrepreneur, ville, statut_professionnel
 * - Conjoint: PAS de cadre_exercice
 */

import type { Lead, SanteProPlusFormData, TransformResult, TransformError, TransformWarning } from './types';
...
export class LeadTransformer {
  /**
   * Transforme un lead AssurProspect en données Alptis Santé Pro Plus
   */
  static transform(lead: Lead): SanteProPlusFormData {
    console.log('\n========================================');
    console.log('LEAD TRANSFORMATION START (SANTÉ PRO PLUS)');
    console.log('========================================');
    ...
    // Note: Santé Pro Plus est réservé aux TNS uniquement
    console.log('\n[ELIGIBILITY] Checking eligibility (TNS only for Santé Pro Plus)...');
    ...
    if (eligibility.shouldSwap) {
      console.log('\n[SWAP] ⚠️  Adhérent non éligible mais conjoint éligible (TNS)');
      ...
      console.log('\n[ELIGIBILITY] ⚠️  WARNING: Ni l\'adhérent ni le conjoint ne sont TNS');
      console.log('[ELIGIBILITY] → Transformation continue (Santé Pro Plus réservé aux TNS)');
    } else {
      console.log('[ELIGIBILITY] ✓ Adhérent éligible (TNS), pas de swap nécessaire');
    }
    ...
    const result: SanteProPlusFormData = {
      mise_en_place,
      adherent,
      ...(conjoint && { conjoint }),
      ...(enfants && { enfants }),
    ...
    console.log('\n========================================');
    console.log('LEAD TRANSFORMATION COMPLETE (SANTÉ PRO PLUS)');
    console.log('========================================');
